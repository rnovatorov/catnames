...
class Game(BaseGame):
    async def start(self):
        ...
        await self._broadcast(message=winner, keyboard=Keyboard.empty().dump())
    ...
    async def reveal_map_to_spymasters(self):
        ...
        await self._send(peer_ids=peer_ids, message=self.map.as_emojis())
        ...
        async with self._sub_for_messages() as messages:
            async for msg in messages:
                ...
                try:
                    ...
                    await self._broadcast(message="Такой клетки нет.")
                    continue

                if cell.flipped:
                    await self._broadcast(message="Клетка уже перевернута.")
                    continue
